...
function validateLegacyDerivedFiles({ sourceFile, amendmentFile, effectiveFile }) {
  const loadErrors = [];
...
function main() {
  ...
  if (!parsed) {
    console.error("Usage: node scripts/validate_legacy_derived.js --source <source-bundle.json> --amendments <legacy-amendment-mappings.json> --effective <legacy-effective-records.json>");
    process.exit(2);
  }

  const result = validateLegacyDerivedFiles(parsed);
  if (!result.ok) {
    const exitCode = result.configError ? 2 : 1;
    console.error(`Legacy derived validation failed with ${result.errors.length} error(s):`);
    for (const error of result.errors) console.error(`- ${error}`);
    process.exit(exitCode);
  }

  console.log(`Validated ${result.amendmentMappingCount} legacy amendment mapping(s) and ${result.effectiveRecordCount} legacy EffectiveRecord(s).`);
}
