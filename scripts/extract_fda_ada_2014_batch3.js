...
const batch3Data = JSON.parse(fs.readFileSync(path.resolve(__dirname, "fda_ada_2014_batch3_data.json"), "utf8"));

const SOURCE_PDF = "source_pdfs/FDA Immunogenicity-Assessment-for-Therapeutic-Protein-Products.pdf";
...
async function main() {
  console.log("=== Running 3-Pass Extraction on FDA 2014 ADA Batch 3 (Section VI & Appendix A) ===");
  const client = createClient();

  for (const sec of batch3Data.sections) {
    if (!bundle.sections.some((s) => s.section_id === sec.section_id)) {
      bundle.sections.push(sec);
    }
  }

  for (const rawSu of batch3Data.source_units) {
    const su = {
  ...
  const sectionIds = batch3Data.sections.map((s) => s.section_id);
  ...
  console.log("\n==================================================");
  console.log("Successfully completed FDA 2014 ADA Batch 3!");
...
main().catch((err) => {
  console.error("Batch 3 extraction failed:", err);
  process.exit(1);
});
