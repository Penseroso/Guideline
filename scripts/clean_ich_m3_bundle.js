...
const pdfPath = path.resolve(__dirname, "..", "source_pdfs", "ICH M3.pdf");
const pdfBuf = fs.readFileSync(pdfPath);
const checksum = crypto.createHash("sha256").update(pdfBuf).digest("hex").toUpperCase();

const SOURCE_PATH = "source_pdfs/ICH M3.pdf";
  ...
  documents: [
    {
      ...
      guideline_code: "ICH M3(R2)",
      issuing_body: "ICH",
      document_version_label: "Step 4 version, dated 11 June 2009",
...
for (const su of cleanedBundle.source_units) {
  if (su.trace) {
    ...
    su.trace.document_id = "ich_m3_r2";
  }
}
