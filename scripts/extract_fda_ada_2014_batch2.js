...
const batch2Data = JSON.parse(fs.readFileSync(path.resolve(__dirname, "fda_ada_2014_batch2_data.json"), "utf8"));
...
async function main() {
  console.log("=== Running 3-Pass Extraction on FDA 2014 ADA Batch 2 (Section V: V.A & V.B) ===");
  const client = createClient();

  for (const sec of batch2Data.sections) {
    if (!bundle.sections.some((s) => s.section_id === sec.section_id)) {
      bundle.sections.push(sec);
    }
  }

  for (const rawSu of batch2Data.source_units) {
    const su = {
  ...
  const sectionIds = batch2Data.sections.map((s) => s.section_id);
  ...
  console.log("\n==================================================");
  console.log("Successfully completed FDA 2014 ADA Batch 2!");
...
main().catch((err) => {
  console.error("Batch 2 extraction failed:", err);
  process.exit(1);
});
