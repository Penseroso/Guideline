...
async function main() {
  console.log("=== Running 3-Pass Extraction on ICH M3(R2) Batch 4 (Sections 13~19) ===");
  const client = createClient();

  for (const sec of batch4Data.sections) {
    if (!bundle.sections.some((s) => s.section_id === sec.section_id)) {
      bundle.sections.push(sec);
    }
  }

  for (const rawSu of batch4Data.source_units) {
    const su = {
  ...
  const sectionIds = batch4Data.sections.map((s) => s.section_id);
  ...
  console.log("\n==================================================");
  console.log("Successfully completed ICH M3(R2) Batch 4 (100% Ingested)!");
...
main().catch((err) => {
  console.error("Batch 4 extraction failed:", err);
  process.exit(1);
});
