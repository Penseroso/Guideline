/**
 * Stage D hierarchy-derived semantic overlay authoring.
 *
...
function ensureSectionFacet(overlay, sectionsById, sectionId, parentFacetId = null) {
  ...
  if (!section) throw new Error(`Unknown Stage D section ${sectionId}`);
...
function main() {
  ...
  for (const documentId of Object.keys(PARENT_SECTIONS)) {
    ...
    // Removed after the first Stage D verification exposed that Part I and
    // Part II both contain a section named "Notes". This exact object was
    // generated by Stage D and is replaced by path-qualified facet ids.
  ...
  if (total !== 55) throw new Error(`Stage D inventory mismatch: expected 55 unique manifests, found ${total}`);
  console.log(`Stage D inventory: ${total} unique manifests`);
}
