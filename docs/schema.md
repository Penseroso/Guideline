...
`scripts/run_semantic_stage_e1_audit.js` (`npm run audit:semantic:stage-e1`) offline-verified that all 5 pre-existing pilot `summary_specs` reach their intended manifest (5/5) — including the containment-match case (ich_m3_r2/ich_s6_r1's §1.3-targeted "scope" summary attaching to the broader §1 "section_1_introduction" manifest, since the summary's sole facet is one of that manifest's coverage-group facets). `scripts/promote_semantic_stage_e1.js` requires that audit, schema validation, a complete live 50-question run on contract `2.3.0`, and the same established-16-suitable-case regression guard Stage D used, before flipping any `summary_spec.review_status` to `reviewed`. No environment with LLM API access was available to run that live audit as of this writing, so engineering completion is done but the 5 summary_specs remain `needs_review` — the same engineering-completion/final-promotion split Stage D documented. Full record: `history/verification/semantic_stage_e1_2026-09-08.md`.
...
`scripts/run_semantic_stage_e2_audit.js` (`npm run audit:semantic:stage-e2`) offline-verified all 3 existing presentation files (ema_fih, ich_m3_r2, ich_s6_r1) render in the correct sentence-role order (3/3); `fda_ada`/`fda_ada_2014` have a `summary_spec` but no presentation file yet, so they correctly serve `text: null` and are out of this audit's scope (Stage F). `scripts/promote_semantic_stage_e2.js` requires the same live-audit-plus-regression-guard gate as Stage E1 (contract `2.4.0`); with no LLM API access available here, the 3 presentation entries remain `needs_review`. Full record: `history/verification/semantic_stage_e2_2026-09-08.md`.
...
`scripts/run_semantic_stage_e3_audit.js` (`npm run audit:semantic:stage-e3`) offline-verified all 7 existing salience_profiles (across all 6 pilot documents) attach to their intended manifest (7/7). `scripts/promote_semantic_stage_e3.js` requires the same live-audit-plus-regression-guard gate as Stage E1/E2 (contract `2.5.0`); with no LLM API access available here, the 7 salience_profiles remain `needs_review`. Full record: `history/verification/semantic_stage_e3_2026-09-08.md`.

This completes Stage E's wiring and offline verification for all three objects Stage D left unconnected. Live-audit-gated final promotion is pending for all of Stage E0–E3's data; extending summary_specs/presentation/salience authoring to Stage D's 47 newly generated manifests is separate future work (Stage F).
