...
**Automated binding pipeline (`engine/binding_agent.js`)** — same extraction/verification separation `product_roadmap.md` §2.5.1 already established for the source archive, applied to this derived layer instead of a manual/seeded process: (1) LLM proposes `bindability`/`non_bindable_reason`/`binding_role`/predicate/`evidence_span` for one `Condition`; (2) a deterministic gate checks schema shape, `evidence_span` substring, and slot/value vocabulary membership; (3) a separate verification-agent call (reusing `verification_agent.js`'s `verifyClaim`, not a new entailment mechanism) checks the predicate's natural-language restatement is actually entailed by `condition_text`; (3b) a separate **completeness gate** (`checkCompletenessGate`) checks the restatement doesn't *omit* a named alternative or the source's real conditioning fact — added after a live semantic audit found 6 of 15 bindable predicates lost real information despite passing entailment, since entailment only catches an added claim, never a dropped one (docs/milestone_log.md M6 "Binding semantic audit + pipeline hardening"); checked against the full `condition_text`, not `evidence_span`, since `evidence_span` is itself the model's own selection and can be the too-narrow thing; (4) a proposed `full_scope` role is never trusted from step 1 alone — it must additionally pass a stricter, separate gate or is demoted (never rejected) to `partial_scope`; (5) `verification_status="verified"` only if every gate passed, otherwise `needs_review` — kept and visible, never discarded. The S6(R1) slice runs through this same pipeline first and is used as a regression fixture for the EMA FIH and FDA ADA slices that follow, but — being agent-authored rather than human-curated — is not treated as golden ground truth the way the hand-reviewed source archive is.

**A `non_bindable` classification with `verification_status="needs_review"` is a usable ontology-gap signal**: cross-referencing all 56 `non_bindable` bindings against `verification_status` found that all 4 `non_bindable`+`needs_review` cases were independently identifiable, by manual read, as likely missing-slot candidates rather than genuine hedges — the model's own uncertainty correlates with exactly the cases worth reviewing for a new slot. Recorded as a standing discovery heuristic for future ontology work, not a one-off observation.

**Context ontology v0.2** (docs/milestone_log.md M6 "Context ontology v0.2"): `disease_severity` (`general | severe_or_life_limiting`) added to `program_finding_slots`, the only audit-found candidate that cleared the 2+-independent-occurrence promotion bar. `prior_healthy_volunteer_data`, `multi_domain_product`, and `high_homology_protein_family` remain unpromoted (each below the bar in the current 71-condition sample); an assay-level statistical-behavior axis (mean/variance patterns) clears the occurrence bar but was deliberately not added to `program_slots`/`program_finding_slots` at all — it describes a property of an assay's *data*, not the *program*, and is left as an open design question (a possible second context category, e.g. `assay_data_state`) rather than force-fit into the existing vocabulary. Adding the one evidenced slot needed zero incidental additions elsewhere — vocabulary after the addition is exactly 7 pre-existing slots + 1 deliberate addition, not a cascade.
