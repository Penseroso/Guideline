...
A separate, additive proposal for a source-grounded answer-planning overlay is documented in `docs/derived_semantic_layer.md`. It does not calculate amendment/effective state and does not change this `0.5.0` model. Its Stage A contract (schemas, validator, samples for all 6 pilot documents), Stage B shadow-mode wiring, and Stage C (all 5 coverage_manifests and both comparison_bindings promoted to `reviewed`; disclosure-only, on the `grounded_generation` and `structured` routes) are implemented — see "Derived semantic overlay" below.
