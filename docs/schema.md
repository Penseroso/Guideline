...
Full design and rationale: `docs/derived_semantic_layer.md`. This section registers the implemented Stage A–D data layer, per that document's §12 checklist.
...
Semantic overlay contract `0.3.0` requires every facet to declare `coverage_basis` and every `salience_profile` to declare `review_status` (added in Stage E0, so a profile can be gated `reviewed`/`needs_review` the same way every other overlay object already was). `declared_members` makes its explicit member record set the effective denominator; `section_census` uses its scoped section census, including one parent-body bucket when substantive records are directly filed on a chapter that also has children. Runtime coverage exposes `effective`, `exact`, and `section`, but computes status only from `effective`. The validator rejects an empty `declared_members` denominator and a `section_census` facet whose scope is not a section. Answer envelope `2.2.0` is the corresponding public API contract.
...
`scripts/run_semantic_shadow_audit.js` replays the already-captured 50-question answer-suitability audit envelopes through this comparison without re-running the LLM, and `history/verification/semantic_shadow_stage_b_2026-09-03.md` records both the first pass and a same-day structural revision: the shadow plan independently reproduced the Q06 (assay-technique branch ambiguity) and Q26 (document-overview collapsed to one section) findings from the human audit, and the first pass surfaced five algorithm-level gaps (not per-question quirks) — relevance matching missed sibling sections, facet coverage depended entirely on one hand-curated sample record per facet, comparison bindings were reported with no check that either side had actual evidence, the existing engine's own presentation order was never captured for the "record both plans' order side by side" requirement, and a stale overlay was indistinguishable from one that was never authored. All five are now fixed in `engine/semantic_shadow.js`/`engine/semantic_overlay_store.js` (a per-section record census replaces the single-sample coverage denominator; sibling-section relevance; per-side comparison-binding coverage with a `both_sides_evidenced` flag; `existing_plan.claim_order`; a distinct `overlay_stale` reason) and covered by `test/engine_semantic_shadow.test.js`. Verifying the new census against real section sizes then surfaced a sixth gap — a facet scoped to a whole multi-subsection chapter (e.g. `ema_fih`'s `dose_selection`, §7, ~197 records across 7 sub-sections) got a record-recall denominator no real answer could approach, reading as permanently near-zero regardless of answer quality. `measureSectionCoverage` now switches denominator shape based on whether the facet's scope section actually has child sections: leaf-scoped facets keep record-level recall, chapter-scoped facets measure "how many of its own named sub-sections were touched" instead (`granularity: "section"` vs `"record"` on the reported `section` field) — a rule derived from the core section tree shape, not hardcoded per facet.

Two more pieces close out Stage B: `test/engine_semantic_shadow_regression.test.js` drives real questions through the actual (offline, deterministic) engine rather than hand-built envelopes, so a future regression in routing/retrieval itself — not just in `engine/semantic_shadow.js` — has a test that would catch it. `scripts/check_semantic_overlay_promotion.js` (`npm run check:promotion`) is a read-only worksheet against `docs/derived_semantic_layer.md` §11's activation criteria: it checks what's mechanically checkable today (validator pass, no stale documents, how often each manifest/binding was actually exercised in the latest shadow audit replay) and lists the remaining criteria that need Stage C to exist before a human can judge them. It never writes `review_status`. Stage A was then expanded to cover all 6 guideline documents and the two remaining 부적합-rated audit questions (`fda_ada.json`'s `screening_performance` manifest, and `fda_ada_2014.json` — that document's first overlay); see `history/verification/semantic_shadow_stage_b_2026-09-03.md` §9 for the replay results and two more general algorithm fixes that expansion forced (a chapter-scoped facet with zero curated members could never reach `"covered"`; a facet-targeted overview manifest could go unreported when the router resolved sections sharing no ancestor/sibling with it at all — the exemption is now keyed on the manifest's own `document_overview`/`topic_overview` answer_intent rather than only `target.type=document`).

### Stage C

`docs/derived_semantic_layer.md` §10's activation rule — "reviewed이고 hash가 최신인 객체만 답변에 사용한다" — makes promotion a verification-pipeline decision. Stage A first promoted 5 coverage manifests and two `scope.product_or_matrix` comparison bindings. `engine/semantic_shadow.js`'s `buildReviewedSemanticCoverage()` reuses Stage B's `buildShadowPlan()` unchanged, filters manifests to `reviewed` only, and filters comparison bindings the same way (keeping an axis only when >=2 documents still have a reviewed binding on it afterward). `engine/answer_envelope.js` calls it — wrapped so a failure here can never affect the served answer — on both the `grounded_generation` success path and the `structured` route's deterministic path (several manifests are exercised mostly via `structured`, e.g. `ich_m10.sem.manifest.run_acceptance`'s real audit case comes back as `route:"structured", mode:"multi_criterion"`), attaching the result as `envelope.semantic_coverage` without ever touching `prose`, `claims`, or the structured citation contract below it. `refusal`/`source_excerpts` routes are deliberately excluded — there's no answer to disclose coverage against on the former, and the latter is already a degraded verbatim fallback. `web/render.js`'s `renderSemanticCoverage` renders a distinct disclosure box (separate from the pre-existing coarse `coverage-warning`) for both manifest facet-coverage and comparison-axis entries, only when present. Full writeup, including live-browser verification on all three paths (grounded_generation, structured, structured/comparison): `history/verification/semantic_stage_c_pilot_2026-09-03.md`.

### Stage D

Stage D0 migrated schema, engine, UI, existing overlays, and the public answer envelope before new authoring. Stage D then generated a final, de-duplicated inventory of **55 unique coverage manifests**: 6 document overviews + 42 substantive parent sections + 5 leaf process/conditional topics + 2 independent specialized manifests. Of the 5 pre-existing manifests, EMA FIH document overview, FDA ADA assay validation, and FDA 2014 risk factors were absorbed/migrated into the hierarchy; only M10 run acceptance and FDA ADA screening performance remain separate specialized objects.

`scripts/build_semantic_stage_d.js` performs hierarchy-driven authoring, `scripts/run_semantic_stage_d_audit.js` exercises all 55 manifests after authoring, and `scripts/verify_semantic_stage_d.js` independently checks hierarchy topology, document areas, leaf membership, staleness, schema invariants, and final uniqueness. Served selection is narrower than shadow diagnostics: it ranks reviewed candidates by the section/topic explicitly named in the question, compatible answer intent, then resolved evidence distance. `scripts/promote_semantic_stage_d.js` requires a complete live 50-question contract-2.2.0 audit, exact no-regression checks for the established 16 suitable cases, and an explicit remaining-set review attestation before writing `reviewed` status.

The 2026-09-08 audit completed 50/50 with 16 suitable, 34 partially suitable, and 0 unsuitable; the established 16 retained identical route, mode, and claim IDs. All 55 manifests are reviewed and the post-authoring audit selected 55/55. If live execution is unavailable, engineering completion and final reviewed promotion remain distinct: the former may pass deterministic verification while the latter stays pending. Stage D does not activate `summary_specs`, semantic presentation sentences, or salience profiles at runtime.

### Stage E0

Stage D left three overlay objects unconnected at runtime: `summary_specs`, the Korean presentation overlay, and `salience_profiles`. These are being activated one at a time (Stage E1/E2/E3, in progress). Before any of them could be gated the same way every other overlay object already is, `salienceProfile` needed a `review_status` field — unlike `summary_specs`/`facets`/`coverage_manifests`/`comparison_bindings`, it had none, so a "reviewed-only" serving rule had no field to filter on. Stage E0 is that one schema change: `data/schemas/derived_semantic_overlay.schema.json` now requires `review_status` on every `salienceProfile`, and `semantic_overlay_version` moved to `0.3.0`. `scripts/migrate_semantic_overlay_v0_3.js` migrated all 6 committed overlays, setting every existing `salience_profile.review_status` to `needs_review` (none had ever been through review). This stage changes only the overlay schema/data — it does not yet change `engine/answer_envelope.js`'s `ENVELOPE_VERSION` (`2.2.0`), since no envelope-visible shape changed.

### Stage E1

`engine/semantic_shadow.js`'s `buildManifestPlan()` now also attaches `selectServedSummary(overlay, manifest)` to every manifest it builds — the matching summary_spec if one exists, carried with its own `review_status` (shadow-diagnostic, unfiltered, same as every other object here). `buildReviewedSemanticCoverage()` then strips that down to `summary: null` unless `review_status === "reviewed"`, exposing `{ summary_id, summary_kind, facet_ids, sentence_roles, text: null }` — `text` stays `null` pending Stage E2's presentation-sentence wiring. `web/render.js`'s `renderSemanticCoverage` reorders a manifest's disclosed facets by `summary.facet_ids` when present, falling back to declaration order otherwise. `ENVELOPE_VERSION` moved `2.2.0` → `2.3.0` for this additive `semantic_coverage.manifests[].summary` field.

`scripts/run_semantic_stage_e1_audit.js` (`npm run audit:semantic:stage-e1`) offline-verified that all 5 pre-existing pilot `summary_specs` reach their intended manifest (5/5) — including the containment-match case (ich_m3_r2/ich_s6_r1's §1.3-targeted "scope" summary attaching to the broader §1 "section_1_introduction" manifest, since the summary's sole facet is one of that manifest's coverage-group facets). `scripts/promote_semantic_stage_e1.js` requires that audit, schema validation, a complete live 50-question run on the current answer contract (checked against the live `ENVELOPE_VERSION`, not a hardcoded string, since by the time all of E1-E3 share one codebase there is no way to audit "as of E1 only"), and the same established-16-suitable-case regression guard Stage D used, before flipping any `summary_spec.review_status` to `reviewed`. Promoted to `reviewed` on 2026-09-08 against a fresh 50/50 live audit on contract `2.5.0`; one established case (Q25) showed a differing `grounded_generation` claim on rerun, confirmed via reproduction and content review to be model-generation variance unrelated to any Stage E code change (`history/decision_log/review_log.md` REV-015), not a real regression. Full record: `history/verification/semantic_stage_e1_2026-09-08.md`.

### Stage E2

`engine/semantic_overlay_store.js` now filters `data/derived/presentation/ko/*.json` for evidence freshness at load time (previously unchecked there — only the validator caught it at authoring time): a presentation entry with any stale `evidence_refs` is dropped whole, not truncated to its still-fresh units, so a "scope" entry can never lose only its exception sentence and read as broader than the source supports. `engine/semantic_shadow.js`'s `presentationTextFor()` finds the presentation entry sharing a summary_spec's `summary_id` as its own `semantic_id`, requires that entry's own `review_status` to be `reviewed` (independent of the summary_spec's own review_status), and orders its units by the summary_spec's `sentence_roles`. `web/render.js`'s `renderCuratedOverview()` renders that text as the `curated-overview` box directly under the section/topic overview header — separate from the pre-existing `synopsisText()` raw-record excerpt, per this document's rule that a presentation summary and `normalized_ko` are never each other's substitute. `ENVELOPE_VERSION` moved `2.3.0` → `2.4.0` for the newly-populated `summary.text` field; the attachment point (after `answerFallback`'s LLM call already returned `prose`/`claims`) makes it structurally impossible for this text to reach a generation prompt.

`scripts/run_semantic_stage_e2_audit.js` (`npm run audit:semantic:stage-e2`) offline-verified all 3 existing presentation files (ema_fih, ich_m3_r2, ich_s6_r1) render in the correct sentence-role order (3/3); `fda_ada`/`fda_ada_2014` have a `summary_spec` but no presentation file yet, so they correctly serve `text: null` and are out of this audit's scope (Stage F). `scripts/promote_semantic_stage_e2.js` requires the same live-audit-plus-regression-guard gate as Stage E1. Promoted to `reviewed` on 2026-09-08 alongside Stage E1/E3, sharing the same live audit run. Full record: `history/verification/semantic_stage_e2_2026-09-08.md`.

### Stage E3

Fixed a latent bug in `buildSaliencePlans()` (`engine/semantic_shadow.js`): it never included `target_id` in its returned shape, so `buildShadowPlan()`'s own `profilePlan.target_id === documentId` document-level branch always compared `undefined` to a string and was dead code since Stage B. `selectServedSalience(overlay, manifest)` matches a `salience_profile` to a manifest using the same exact-target-then-facet-containment pattern `selectServedSummary()` already uses (`target_id` here is a bare id, directly comparable to `manifest.target.id` regardless of target type). `servedSalience()` groups a reviewed profile's items into `{ primary, supporting, detail }` facet-id arrays (each tier sorted by `display_order`); `web/render.js`'s `renderSemanticCoverage` keeps `primary`/`supporting` facets in the normal disclosed list and moves only `detail`-tier facets behind a `<details>` widget — a facet the profile doesn't mention at all stays visible, since salience narrows exposure order, never disclosure itself. `ENVELOPE_VERSION` moved `2.4.0` → `2.5.0` for the new `semantic_coverage.manifests[].salience` field.

`scripts/run_semantic_stage_e3_audit.js` (`npm run audit:semantic:stage-e3`) offline-verified all 7 existing salience_profiles (across all 6 guideline documents) attach to their intended manifest (7/7). `scripts/promote_semantic_stage_e3.js` requires the same live-audit-plus-regression-guard gate as Stage E1/E2. Promoted to `reviewed` on 2026-09-08. Full record: `history/verification/semantic_stage_e3_2026-09-08.md`.

This completes Stage E's wiring, offline verification, and final live-audit-gated `reviewed` promotion for all three objects Stage D left unconnected (5 summary_specs, 3 presentation entries, 7 salience_profiles).

### Stage F

Extends `summary_specs`/`salience_profiles` (structure only — no engine, rendering, schema, or envelope changes, since `selectServedSummary()`/`selectServedSalience()` already generically match any overlay data) to the 47 manifests Stage D added but Stage E's pilot scope didn't cover. `scripts/build_semantic_stage_f.js` mechanically authors a `summary_spec` for every manifest whose `answer_intent` maps to a `summary_kind` (`document_overview`/`section_overview`/`topic_overview`/`process`; `multi_criterion`/`comparison` manifests are excluded — a list-of-criteria or axis-comparison answer isn't the "overview paragraph" shape summary_specs solve for), reusing the manifest's own existing `facet_ids` and resolving `evidence_refs` to a real core record (declared-members facets use `member_record_ids[0]`; section-census facets search the section and its descendants, falling back to the manifest's own target section when every child facet turns out to have zero curated content — a real case, ich_s6_r1 §5's four child sub-sections). `salience_profiles` are generated for manifests with 5+ facets, deriving `tier`/`rationale_code` deterministically from each facet's own already-reviewed `semantic_role` rather than a new editorial judgment.

Korean presentation text authoring is explicitly out of scope for Stage F — a sample of parent sections (e.g. ich_m3_r2 §5/§11) have no directly-filed content of their own, so quoting already-reviewed sentences would leave most new summaries empty anyway, and synthesizing new prose across ~46 manifests without a review step carries real accuracy risk; the user chose to defer this to separate manual work. All 45 new summary_specs therefore carry `text: null` at runtime, the same fallback Stage E2 already established.
...
The model was designed to remain usable beyond M10, and now does: ICH M3(R2), S6(R1), EMA FIH, FDA ADA, and FDA ADA 2014 are all structured and reviewed under the same model with no schema change. Those guidelines contain more condition-, exception-, species-, study-type-, and development-stage-centered language than M10's own sections did.
...
An M6 spike ("Regulatory World Model") explored a derived, additive layer that evaluated whether a specific rule (`KnowledgeRecord`/`QuantitativeCriterion`) applies given a structured RegulatoryContext, using the archive's `Condition` records as evidence — moving from "search guidelines and answer" toward "given this regulatory context, which rules apply and why." It included a `Condition`→predicate binding pipeline (LLM-proposed, gated on entailment and a completeness check), a deterministic evaluator, a RegulatoryContext ontology, and rule discovery over a natural-language question.

**Discontinued as a separate module** (`docs/milestone_log.md` M6) after a real-usage review: the verdict architecture (four-value applicability judgment, binding roles, its own CLI) added substantial maintenance surface for a coverage island of 71 of the archive's 279 `Condition` records across 3 guidelines, with no natural-language entry point into the rest of the archive. The code and data are archived, not deleted, at `history/applicability_engine/` — see that directory and the M6 milestone log entries for the full design and the measurements taken (a live semantic audit found 6 of 15 bindable predicates lost real information despite passing entailment; the eventual pipeline needed zero new RegulatoryContext slot types across the three guidelines it was tried on).

Two things it surfaced were kept as direct, additive improvements to this engine, described where they actually live:

- Several Korean regulatory terms found missing from `engine/text_utils.js`'s `REGULATORY_SYNONYMS` while authoring the discontinued layer's slot vocabulary — added directly to that dictionary.
- A `KnowledgeRecord`/`QuantitativeCriterion`'s own attached `Condition`s were already being correctly resolved (the `conditionsByTarget` reverse-index fix, `engine/data_store.js`) but never shown to the user — `engine/query_router.js`'s `formatAnswer` now appends them as a verbatim caveat on every answer, via the answerable record's `applicable_conditions` field.

Model 0.6.0's document/section-level scope ontology (above) is unaffected — the discontinued layer was a rule-level, `Condition`-level addition on top of it, not a change to it.
