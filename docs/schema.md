...
Full design and rationale: `docs/derived_semantic_layer.md`. This section registers the implemented derived semantic overlay as part of the current data model.
...
`engine/semantic_overlay_store.js` loads these overlays at server startup, dropping any document whose `source_bundle_sha256` no longer matches the live core bundle. `engine/semantic_routing.js` has two entry points over that store: `buildReviewedSemanticCoverage()` — called by `engine/answer_envelope.js` on both the `grounded_generation` and `structured` success paths, filtering every object to `reviewed` only and attaching the result as `envelope.semantic_coverage` (never touching `prose`, `claims`, or the citation contract; wrapped so a failure here can never affect the served answer) — and `buildShadowPlan()`/`comparePlans()`, a diagnostic-only side-by-side plan `engine/server.js`'s `/api/ask` handler logs to `logs/runtime/semantic_shadow.jsonl` (`engine/semantic_shadow_log.js`) strictly after the real envelope is already built, never served. `refusal`/`source_excerpts` routes are excluded from `semantic_coverage`. `web/render.js`'s `renderSemanticCoverage` renders manifest facet-coverage and comparison-axis disclosure (with `detail`-tier salience facets collapsed behind a `<details>` widget — a facet a profile doesn't mention stays visible; salience narrows exposure order, never disclosure itself), and `renderCuratedOverview()` renders a summary's reviewed Korean presentation text separately from the raw-record excerpt box.

Selection for both entry points ranks reviewed candidates by the section/topic explicitly named in the question, compatible answer intent, then resolved evidence distance — never by question frequency or a fixed test set.

`scripts/check_semantic_overlay_promotion.js` (`npm run check:promotion`) is a read-only worksheet against §11's activation criteria; it never writes `review_status`. `test/engine_semantic_routing_regression.test.js` drives real questions through the actual offline engine (not hand-built envelopes) so a regression in routing/retrieval itself, not just in `engine/semantic_routing.js`, has a test that catches it.

### Current inventory and operation

**55 unique coverage manifests** across all 6 documents: 6 document overviews + 42 substantive parent sections + 5 leaf process/conditional topics + 2 independent specialized manifests (`ich_m10.sem.manifest.run_acceptance`, `fda_ada.sem.manifest.screening_performance`). **50 applicable `summary_specs`** (of 55 manifests, 4 `multi_criterion`/`comparison`-intent manifests have no summary_spec — that answer shape doesn't fit an "overview paragraph") and **26 `salience_profiles`** (manifests with 5+ facets). **All 50 applicable summary_specs have a reviewed Korean presentation entry with 0 explicit facet gaps** (`npm run audit:semantic:presentation`).

Each presentation entry resolves to one `summary_spec`, carries `summary_spec_sha256` (detects target/kind/facet/role drift independent of source-evidence hashes), and exhaustively classifies that summary's `facet_ids` in `facet_dispositions` — `covered` with `gap_reason: null`, or `gap` with `no_structured_evidence`, `insufficient_support`, or `verification_failed`; a reviewed entry may retain only `no_structured_evidence`, and any hash mismatch (source evidence or summary-spec) drops the whole entry at both validation and runtime.

Building/verifying/promoting this inventory uses `build:semantic:manifests`/`audit:semantic:manifests`/`verify:semantic:manifests` (facets/coverage_manifests/comparison_bindings), `build:semantic:summaries`/`audit:semantic:summaries`/`promote:semantic:summaries` (summary_specs/salience_profiles), and `author:semantic:presentation`/`audit:semantic:presentation`/`promote:semantic:presentation` (Korean text) — see `docs/derived_semantic_layer.md` §10 for the full operational table and `scripts/semantic_promotion_lifecycle.js` for the shared prepare/verify/finalize/rollback promotion lifecycle. How this inventory was originally built, migrated, and promoted (Stage A through G) is recorded in `history/milestones/derived_semantic_layer_stages_a_g_2026-09-08.md` — not needed to operate or extend it today.
