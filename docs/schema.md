...
`scripts/run_semantic_stage_e3_audit.js` (`npm run audit:semantic:stage-e3`) offline-verified all 7 existing salience_profiles (across all 6 pilot documents) attach to their intended manifest (7/7). `scripts/promote_semantic_stage_e3.js` requires the same live-audit-plus-regression-guard gate as Stage E1/E2. Promoted to `reviewed` on 2026-09-08. Full record: `history/verification/semantic_stage_e3_2026-09-08.md`.
...
`scripts/run_semantic_stage_f_audit.js` offline-verified all 45 new summary_specs and 19 new salience_profiles reach their intended manifest (45/45, 19/19). `scripts/promote_semantic_stage_f.js` uses the same gate as Stage E1-E3; since Stage F makes no engine-behavior change, it reused the same live 50-question audit run (and the same reviewed Q25 exception) Stage E0-E3's promotion already produced. Promoted to `reviewed` on 2026-09-08: 45 summary_specs, 19 salience_profiles (bringing the totals to 50 and 26 respectively). Full record: `history/verification/semantic_stage_f_2026-09-08.md`.
