...
- **M5 — Local-first production MVP. Core scope done (2026-08-28), semantic routing revised 2026-09-01.** Delivered the HTTP API, minimal web UI, authentication, monitoring aggregation, feedback flow, answer-page readability, and security hardening. The public response contract now uses semantic routes (`structured`, `grounded_generation`, `source_excerpts`, `refusal`); local source retrieval is key-independent, and one OpenAI key can use distinct generator/verifier models. Remaining work is live cross-model and cross-provider verification, post-M1 extraction-accuracy re-measurement, and deployment/TLS/retention decisions if the product moves beyond local-first use.
