...
- **M6 — Regulatory World Model / Applicability Engine spike (started 2026-08-25).** A separately-scoped, additive spike (not a TPP change — §1.4's non-goal boundary is unchanged; this evaluates "does this rule's own stated scope include this context," never regulatory suitability) evaluating whether a specific rule applies given a structured RegulatoryContext, using the existing `Condition` graph as evidence. Full design in `docs/schema.md` "Applicability Layer 0.1.0"; progress in `docs/milestone_log.md` M6. Scoped to ~71 conditions across ICH S6(R1) (all), EMA FIH §7.2/§7.3, and FDA ADA §IV.A.1/§VI.B — the goal is validating that the applicability model generalizes across three guideline shapes, not full condition-graph coverage.
