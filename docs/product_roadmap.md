...
- **M6 — Regulatory World Model / Applicability Engine spike. Done (2026-08-25/26).** A separately-scoped, additive spike (not a TPP change — §1.4's non-goal boundary is unchanged; this evaluates "does this rule's own stated scope include this context," never regulatory suitability) evaluating whether a specific rule applies given a structured RegulatoryContext, using the existing `Condition` graph as evidence. Full design in `docs/schema.md` "Applicability Layer 0.1.0"; full narrative in `docs/milestone_log.md` M6; measured numbers in `docs/test_record.md` Entry 006. Scoped to 71 conditions across ICH S6(R1) (all), EMA FIH §7.2/§7.3, and FDA ADA §IV.A.1/§VI.B — the goal was validating that the applicability model generalizes across three guideline shapes, not full condition-graph coverage, and it did: 0 new RegulatoryContext slot types were needed across any of the three guidelines. `npm run applicability` (propose/evaluate) and `npm run eval:applicability` are the new entry points. Coverage of the remaining ~208 archive-wide conditions is a candidate for a future milestone, not part of this spike.
