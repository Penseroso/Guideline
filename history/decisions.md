...
### DEC-049: Archive superseded Phase 3/4 planning to `history/`, adopt product roadmap, reprioritize data structuring before application layer

- Date: 2026-08-18
- Status: Accepted
- Decision: The Phase 0-4 plan's non-goal boundary excluding search, embeddings, RAG, and any application layer is superseded; `working_docs/product_roadmap.md` is the active forward plan. Superseded planning documents (`phase3_plan.md`, `phase4_plan.md`, `phase4_handoff_plan.md`, `phase4_module_4_1.md`, `pilot_scope_S6_R1.md`, `pilot_review_M10.md`, `pilot_review_S6_R1.md`, `amendment_effective_strategy.md`, `amendment_prototype_S6_R1.md`, `effective_state_prototype_S6_R1.md`, `derived_contract_module_3_6.md`, `derived_layer_validator_module_3_5.md`) are moved to `history/` as historical rationale, not active plans. An independent audit (`working_docs/audit_2026-08-18.md`) found actual reviewed source-layer coverage under 10% of the two present guidelines (2 of ~70 M10 sections, 4 of ~41 S6(R1) sections), against 61 prior governance entries (49 decisions + 12 reviews) for ~162 structured records. Given that, coverage/data-structuring work on the PDFs already in `Guideline Files/` is reprioritized ahead of any chatbot/retrieval/application-layer milestone; the two M10 "structural pressure probe" pilot files (`m10_phase2_table_pressure.json`, `m10_phase2_reference_condition_pressure.json`) are kept, not discarded, as the seed for the first coverage-expansion unit (DEC-020 already flagged them for eventual absorption). The derived contract `0.1.0` layer (`structured_data/schemas/derived/`, `scripts/validate_derived.js`) is kept but paused — not extended further until real second-guideline derived data exists to justify it. CI (`.github/workflows/validate.yml`) is extended to also run `validate:derived` and `validate:legacy`, closing a gap where those checks passed locally but were never gated in CI.
- Rationale: The regulator-neutral engine and derived-layer speculative build-out (Phase 4) outpaced actual archived content, and the non-goal list conflated "don't build an application layer" (an implementation-sequencing choice, now reversed given the product direction) with "don't give regulatory advice" (a product-boundary choice, unchanged — see `working_docs/project_scope.md` Non-Goals). Building retrieval/chat on 6 leaf sections would produce a demo, not a usable tool; the actual bottleneck is coverage, not application architecture.
- Consequences: No source model, JSON Schema, or existing validated data changes. `history/README.md` records what moved and why. `working_docs/project_scope.md`, `README.md`, and `AGENTS.md` are updated to match. Next work should extract sections `6.2`, `7.6`, `7.6.1`, `8`, `8.2` of ICH M10 to full pilot-quality bundles (superseding the two probe files) before further M10/S6(R1) sections, per `working_docs/product_roadmap.md` §3 Milestone M1.
- Related files: `working_docs/product_roadmap.md`, `working_docs/audit_2026-08-18.md`, `history/README.md`, `README.md`, `AGENTS.md`, `working_docs/project_scope.md`, `.github/workflows/validate.yml`.

### DEC-050: Adopt scale-ready retrieval and extraction primitives from M1, given a multi-regulator eventual target

- Date: 2026-08-18
- Status: Accepted
- Decision: The eventual target is a multi-regulator archive (ICH, FDA, EMA, OECD), reached incrementally rather than in one pass. Two implementation choices from DEC-049/`product_roadmap.md` §2.4.1 are revised to avoid a migration event once incremental additions accumulate: (1) M1 indexes structured records in an embeddable vector store (e.g., LanceDB/Chroma/sqlite-vec) behind a retrieval interface, not a plain in-memory array; (2) M1 also builds and dry-runs a schema-constrained LLM extraction pipeline — using the existing `structured_data/schemas/guideline_bundle.schema.json` as the extraction target schema, with `source_text` always copied verbatim from mechanical PDF extraction (never LLM-generated), and all LLM-drafted records starting at `review_status=needs_review` for human verify/correct rather than classify-from-scratch. Dedicated graph-database products and further extension of the paused derived-contract layer (`structured_data/schemas/derived/`) remain deferred; the existing ID-referenced JSON graph is already additive and has no equivalent migration trap.
- Rationale: Manual curation at the audited governance ratio (~1 decision per 2.5 records, `working_docs/audit_2026-08-18.md` §3) cannot cover a multi-regulator corpus regardless of retrieval technology; an in-memory array retrieval layer would need replacing once volume grows, which is a real, avoidable migration cost. Both changes cost little at current scale (6 sections) and remove a predictable future rewrite.
- Consequences: `working_docs/product_roadmap.md` §2.5 and §3 (Milestone M1/M3) updated accordingly. No source model or JSON Schema change; the existing schema becomes the LLM extraction contract as-is. Document-format handling for non-ICH sources (FDA/EMA/OECD) is explicitly not designed yet and is deferred to when a real such document is in scope.
- Related files: `working_docs/product_roadmap.md`, `working_docs/decisions.md` (DEC-049), `structured_data/schemas/guideline_bundle.schema.json`, `scripts/validate_structured_data.js`.
