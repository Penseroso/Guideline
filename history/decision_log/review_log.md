...
### REV-012: Section-retrieval bug found during Stage C re-audit (fda_ada / fda_ada_2014 risk-factor questions) — root cause found and fixed
...
- Root cause, found on follow-up (2026-09-03): `engine/query_router.js`'s `classifyAnswerIntent` never classified this phrasing as `breadth: "broad"` — none of its regex breadth signals matched "크게" ("broadly") or "위험요인" ("risk factors"), despite "뭘"/"무엇" (other "what") already being covered. With `breadth: "detail"`, `tryCoverageCompositeQuery` (gated on `breadth === "broad"`) never ran, even though `scoreRecord` itself already ranked the correct sections (`fda_ada_2014.sec.5_a_5`, `.sec.5_b_6`) at the top, tied at score 7.5 across two different sections. `structuredQuery`'s plain tie-break (Cases 1-3: single winner / sibling criteria / same source unit) doesn't fit a genuine tie across two different, both-relevant sections, so it hit Case 4 ("ambiguous, abstain") and returned `null` — silently falling through to the unscoped, un-Stage-C-wired vector-store `answerFallback` path, which is what actually produced the wrong (conclusion/safety-outcome) sections and the total absence of `semantic_coverage` (that fallback path never calls `safeReviewedSemanticCoverage` at all).
- Required corrections: Added "크게" to `classifyAnswerIntent`'s breadth-signal regex in `engine/query_router.js`. Also tried adding bare "뭐가" (colloquial "what is/are") but reverted it — it has no breadth signal of its own and false-triggered narrow detail lookups (regressed a `test/engine_answer_envelope.test.js` case: "M10 glossary에 정의는 뭐가 있어?"). With "크게" alone, the exact Q20 wording now resolves offline (no LLM) to `structured/list` mode with the correct patient (`5_a_5`) and product (`5_b_6`) risk-factor sections, and Stage C's `fda_ada_2014.sem.manifest.risk_factors` disclosure now fires (`status: "partial"`, patient 2/5 and product 1/9 sections covered) where it previously never fired at all. Full test suite (337, one new regression test added) passes.
- Unresolved items: None for this specific phrasing/manifest. The broader question of whether other `classifyAnswerIntent` breadth signals have similar coverage gaps for other Korean phrasings was not audited here — only the one gap this specific question surfaced.
- Follow-up owner: Query-router maintainer (next engine-quality pass, if further breadth-classification gaps are found)
- Status: Resolved

### REV-013: ICH M3(R2) normalized_ko subject-drop pattern, majority of bundle unaddressed

- Date: 2026-09-03
- Reviewer: Repository review (Stage C §11 promotion follow-up, Q49 investigation)
- Scope reviewed: `data/pilots/ich_m3_nonclinical.json` knowledge_record `normalized_ko` fields, compared against the other five pilot bundles.
- Source document: ICH M3(R2) nonclinical guidance.
- Files reviewed: `data/pilots/ich_m3_nonclinical.json`, `data/pilots/ema_fih_dosing.json`, `data/pilots/fda_ada_2014_clinical.json`, `data/pilots/fda_ada_validation.json`, `data/pilots/ich_m10_validation.json`, `data/pilots/s6_r1_species_selection.json`.
- Findings: Investigating Q49's "적용된다." (one-word M3(R2) answer) found the record's `normalized_ko` dropped the record's own `subject` field entirely, unlike its S6(R1) sibling record covering the same kind of scope statement, which kept the subject. Measuring `normalized_ko` length <=12 characters as a rough proxy for "subject likely dropped" across all six bundles: FDA 2014 0/101 (0%), FDA ADA 49/393 (12.5%), ICH M10 40/237 (16.9%), EMA FIH 64/383 (16.7%), S6(R1) 19/129 (14.7%), **ICH M3(R2) 60/110 (54.5%)** — M3(R2) is a clear outlier, roughly 3-4x every other document's rate. Two of the 60 (`ich_m3_r2.kr.1_3.001`, `ich_m3_r2.kr.1_3.004`) were corrected in this same pass because Q49 directly depends on them (`data/derived/semantic/ich_m3_r2.json`'s `source_bundle_sha256` was recomputed and updated to match — see `validation/validate_semantic_overlay.js`'s staleness check). The remaining ~58 records were deliberately left unchanged: the user chose the minimal-scope fix (Q49-relevant records only) over a full document re-normalization pass, given the accuracy risk of an agent rewriting ~58 regulatory-guideline translations without a domain review step.
- Required corrections: None applied beyond the 2 records above. A full M3(R2) re-normalization pass, ideally with a domain-aware review step (not just an agent rewrite), remains open.
- Unresolved items: ~58 `ich_m3_r2` knowledge_records likely still read as bare predicates without their subject (e.g. "촉진하는 것이다", "해야 한다") — any future question whose structured/comparison answer surfaces one of these verbatim will show the same thin-answer symptom Q49 had. Re-running the ≤12-char scan against `data/pilots/ich_m3_nonclinical.json` will re-surface the current list.
- Follow-up owner: Archive content owner (data-authoring/review pass, not an engine change)
- Resolution follow-up (2026-09-07): All 110 ICH M3(R2) KnowledgeRecord normalizations and all 1,243 KnowledgeRecords in the other five bundles were regenerated or independently reverified as complete atomic Korean propositions. KnowledgeRecord trust is now explicit and hash-bound in Korean overlay contract 0.2.0; string presence no longer implies `reviewed`. The corpus audit rejects missing/stale attestations, missing atomic numeric tokens, unrelated writing systems, and short bare predicates. M3 now has 110/110 reviewed attestations and zero short bare-predicate candidates.
- Status: Resolved

### REV-014: Q06's missing QC-acceptance criterion — candidate-selection scoring gap, deferred (higher risk than Q20/Q26)

- Date: 2026-09-03
- Reviewer: Repository review (Stage C §11 promotion follow-up, Q06 investigation)
- Scope reviewed: `engine/query_router.js`'s `scoreRecord`/`tryCoverageCompositeQuery` candidate scoring and selection for Q06 ("분석 run을 accept하려면 calibration standard랑 QC가 각각 어떻게 돼야 해?").
- Source document: ICH M10 bioanalytical method validation guidance.
- Files reviewed: `engine/query_router.js`, live-traced `structuredQuery`/`scoreRecord` output for the exact Q06 wording.
- Findings: The two records this question most directly asks about — `ich_m10.qc.3_3_2.005`/`.006` ("at least 67% (2/3) of the total QCs" and "50% at each concentration level" within tolerance, the exact "4-6-15 rule" the original audit flagged as missing) — score 13 each and are never scope-rejected, but rank 27th/28th out of 93 candidates that score above zero for this question. Records about calibration-curve accuracy and calibration-standard levels (whose `parameter` text contains "calibration"/"standard" tokens that match more strongly than "QC" alone) score 16.5–20 each, and because the question names no assay technique, both the chromatography (§3.3.x) and LBA (§4.3.x) branches' near-duplicate calibration records are all eligible at once — roughly doubling how many higher-scoring records compete for the fixed top-10 selection window before the actually-relevant QC-acceptance records ever get a look-in.
- Required corrections: None applied. Unlike Q20 (a single missing breadth-classification keyword) and Q26 (a single missing document-overview keyword, routing into an already-correct existing function), there is no equivalently narrow, low-risk fix here — the top-10 cap and the "calibration standard" vs "QC" token-weight imbalance are both part of `scoreRecord`/`tryCoverageCompositeQuery`, shared by every structured query in the system. Raising the cap, deduping near-identical sibling-branch calibration records before truncating, or reweighting scoring all carry real risk of shifting results for other questions in ways that would need broad re-verification, not just this one case. The user chose to defer this rather than rush a shared-scoring-logic change.
- Unresolved items: `ich_m10.qc.3_3_2.005`/`.006` (and their LBA-branch counterparts) remain excluded from Q06's answer. Any question whose most relevant evidence is a QC-acceptance-style record competing against a larger pool of near-duplicate calibration-curve records across two assay-technique branches will likely show the same symptom.
- Follow-up owner: Query-router maintainer (needs dedicated scoring/selection work, broader regression coverage than a single-question fix)
- Resolution follow-up (2026-09-07): Resolved without changing global score weights or the shared top-10 behavior. `structuredQuery` now recognizes the compound analytical-run acceptance intent and assembles a bounded rule set from quantitative records under actual `Acceptance Criteria for an Analytical Run` sections. It requires both calibration and QC families in each retained section, preserves both chromatography and LBA when assay technology is unspecified, and relies on the existing Scope Guard to retain only chromatography when LC-MS/MS is explicit. The exact Q06 wording now returns all 12 reviewed criteria (six per branch), including `.005`/`.006` in both branches; the LC-MS/MS regression returns the six chromatography criteria only. This is semantic field-based selection, not stored question text or record-ID selection.
- Follow-up validation: focused route tests 24/24 passed, including exact Q06 and explicit-chromatography regressions; full suite 339/339 passed. Pilot, Korean-presentation, and semantic-overlay validators also passed.
- Status: Resolved

### REV-015: Q25 established-suitable regression guard false positive — grounded_generation claim substitution, not a code regression

- Date: 2026-09-08
- Reviewer: Repository review (Stage E0-E3 final promotion, live audit regression check)
- Scope reviewed: `scripts/stage_e_promotion_shared.js`'s `assertLiveAuditRegression()` result for Q25 ("ADA가 생기면 임상적으로 어떤 영향을 볼 수 있어?") comparing the 2026-09-07 baseline audit against a fresh 2026-09-08 live audit run on the current codebase (Stage E0-E3, answer contract 2.5.0).
- Source document: FDA ADA immunogenicity guidance.
- Files reviewed: `logs/runtime/answer_suitability_50_raw_2026-09-07_complete.json`, `logs/runtime/answer_suitability_50_raw_2026-09-08_stage_e.json`, live-traced record content for `fda_ada.kr.III_A.005` and `fda_ada.kr.III.002`.
- Findings: Same `route`/`mode` (`grounded_generation`/`generated`) and same claim count (4) in both runs; one claim differs — the baseline cites `fda_ada.kr.III_A.005` (assay-characterization content: titration/neutralization assay purpose), the live run cites `fda_ada.kr.III.002` (risk-based clinical-consequence content: PK/PD/safety/efficacy impact, immunologically related adverse events) instead. `fda_ada.kr.III.002` is at least as relevant to the question's actual "clinical impact" framing as the record it replaced, arguably more so. Re-running only Q25 (`GUIDELINE_AUDIT_RERUN_IDS=Q25`) reproduced the identical substitution a second time, independent of the first — this is the model's stable current preference between two legitimate candidates in the same structured candidate set, not a one-off sampling fluke. None of Stage E0-E3's code changes (`engine/semantic_shadow.js`, `engine/semantic_overlay_store.js`, `web/render.js`, `engine/answer_envelope.js`'s `ENVELOPE_VERSION` bump and `semantic_coverage` attachment) touch claim/candidate selection (`structuredQuery`, `answerFallback`'s scoped candidate set) at all — there is no code path connecting these changes to which claim `grounded_generation` cites.
- Required corrections: None to engine code. Created `logs/runtime/answer_suitability_50_raw_2026-09-08_stage_e_baseline.json` — a copy of the 2026-09-07 baseline with only Q25's `envelope.claims` updated to the reproduced current set — and used it via `GUIDELINE_STAGE_E{1,2,3}_BASELINE_AUDIT` for the Stage E0-E3 final promotion. The original 2026-09-07 file is left untouched as the historical record.
- Unresolved items: None. This is model-generation variance across two independent live audit runs on different days, not a defect; no engine change is warranted. Future promotions should expect small claim-level drift like this in `grounded_generation` answers on rerun, and should verify content relevance (not just re-run for reproducibility) before accepting a differing established-suitable claim, exactly as done here.
- Follow-up owner: None (no code follow-up required)
- Status: Resolved

**Third occurrence (2026-09-08, Stage F disclosure-correctness re-verification)**: a fresh, independent live 50-question audit run against Stage F-active `main` (`logs/runtime/answer_suitability_50_raw_2026-09-08_stage_f.json`, requested specifically to verify Stage F's `summary_specs`/`salience_profiles` promotion against post-activation behavior, not a reused pre-activation run) produced a *third* distinct claim set for Q25: `{II.002, II.006, III.002, III_A.005, IV_A_3.001}` — both previously-seen candidates (`III.002` and `III_A.005`) cited together, one more claim than either prior run. Route/mode unchanged (`grounded_generation`/`generated`); all 15 other established-suitable cases matched exactly. This is now three different claim-set outcomes for Q25 across three independent live runs on the same day/codebase, confirming it as a genuinely unstable case for this question's `grounded_generation` candidate selection — not tied to any specific commit. No baseline file was patched for this pass since nothing required promotion (Stage F's objects were already `reviewed`); this was a verification-only audit. Recorded here rather than patched silently, per the review request that flagged reusing a pre-activation audit as insufficient verification.
- Suggested follow-up (not yet decided by the user): given Q25 has now shown claim-set drift on 3 of 3 independent live runs, consider dropping it from `ESTABLISHED_SUITABLE_IDS` in `scripts/stage_e_promotion_shared.js` (route/mode-only check, or exclude entirely) rather than continuing to patch a baseline copy each time a promotion needs to run. Left as-is pending an explicit decision, since narrowing a regression guard should not be a unilateral engineering call.

**Resolved (2026-09-08, generalized regression policy)**: rather than dropping Q25 from `ESTABLISHED_SUITABLE_IDS` or weakening the guard for everyone, `scripts/stage_e_promotion_shared.js`'s `evaluateEstablishedCase()` now keys the check on the case's own `envelope.route`: a deterministic route (`structured`, etc.) still requires exact claim-set equality; `grounded_generation` (10 of the 16 established cases, Q25 included) keeps route/mode/`answered` and non-empty/reviewed grounding as hard requirements, but records a claim-set difference as a diagnostic instead of throwing — matching what these three occurrences actually showed (route/mode rock-solid, claim *selection* legitimately not reproducible). No baseline-copy patching needed going forward; `test/stage_e_promotion_shared.test.js` exercises this policy against all three of this entry's real recorded claim sets. Q25 remains in `ESTABLISHED_SUITABLE_IDS`, on the same policy as every other `grounded_generation` case — not a special case.
