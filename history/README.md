...
- `pilot_review/`: initial scoping/review notes from when the M10 and S6(R1) bundles were still called "pilots". The bundle JSON files themselves remain active under `data/guidelines/` (renamed from `data/pilots/`) — only these early process notes are archived.
- `decision_log/`: `decisions.md` (DEC-001–DEC-050) is the full pre-2026-08-18 per-decision governance log, frozen when logging switched to `docs/milestone_log.md` (one entry per roadmap milestone instead of one per decision). `review_log.md` (REV-001–…) is not frozen the same way — it remains the live per-incident detail log a milestone entry links out to (e.g. `docs/milestone_log.md`'s Q25 note points at `review_log.md` REV-015); new entries are still appended there when a specific finding needs a full reviewed writeup.
...
- `build_scripts/`: one-time extraction/cleanup/migration/backfill scripts that built the six committed `data/guidelines/` bundles and their Korean normalizations; not wired to any `package.json` script or required by active code. See its own `README.md`.
