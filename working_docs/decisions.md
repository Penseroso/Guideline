...
- Date: 2026-07-13
- Status: Accepted
- Supersession note: Part (1) is superseded by DEC-053. The production registry no longer references `structured_data/pilots/` directly; it references new canonical minimal Document-identity bundles under `structured_data/source_documents/`, because `working_docs/phase4_plan.md`'s Engine boundary designates Phase 1-3 pilots as non-production inputs. Part (2), the additive multi-bundle manifest mechanism, is unchanged and remains in effect.
...
### DEC-053: Resolve production registry source-Document identity via canonical minimal identity bundles

- Date: 2026-07-13
- Status: Accepted
- Decision: Supersedes DEC-051 part (1). The Module 4.2 production registry's source-Document identity for `ich_m10` and `ich_s6_r1` is provided by new canonical, minimal, Document-identity-only source bundles at `structured_data/source_documents/ich_m10.json` and `structured_data/source_documents/ich_s6_r1.json` — each a source model `0.2.0`-conformant bundle containing only the `documents` collection (one record, byte-identical to the corresponding reviewed pilot's `Document` record) and empty `sections`, `source_units`, `knowledge_records`, `quantitative_criteria`, `conditions`, and `cross_references` arrays. `structured_data/derived/registry/manifest.json` references these two bundles instead of `structured_data/pilots/*.json`. DEC-051 part (2), the additive multi-bundle manifest mechanism (`source_bundles`), is retained unchanged and is exactly what combines these two identity bundles into one source index for registry validation.
- Rationale: DEC-051 part (1) bootstrapped registry source-Document identity from the existing reviewed pilot bundles. That created a direct production dependency on `structured_data/pilots/`, which `working_docs/phase4_plan.md`'s Engine boundary explicitly designates as a non-production input: "Phase 1-3 pilots, probes, prototypes, and review records are regression references and audit history, not production inputs for the runtime contract." A minimal, schema-valid, Document-only bundle resolves this conflict without inventing any new Document fact (the Document record is copied verbatim, not re-derived) and without waiting for full source-layer extraction (Modules 4.4/4.10), which remains the eventual source of the complete production bundles.
- Consequences: `structured_data/derived/registry/manifest.json` no longer references any file under `structured_data/pilots/`. The two new identity bundles are revalidated by `npm run validate:registry` on every run (via the existing source-bundle revalidation in `scripts/validate_derived.js`) and are schema- and cross-object-valid under the unmodified source JSON Schema and `scripts/validate_structured_data.js`. When Modules 4.4/4.10 produce complete production source-layer bundles for `ich_m10` and `ich_s6_r1`, those bundles carry the same `document_id` and checksum and may replace these identity bundles as the registry manifest's source, or the registry manifest may reference the complete bundles directly; either path preserves registry identity without change. `structured_data/source_documents/` is a new top-level source-layer directory, distinct from `structured_data/pilots/` (reviewed section-level content, regression references, not production inputs) and from the `structured_data/bundles/<document_id>/` path reserved for Module 4.3/4.10 full per-document outputs (structure manifest, coverage ledger, complete bundle); it holds only minimal canonical Document-identity bundles.
- Related files: `structured_data/source_documents/ich_m10.json`, `structured_data/source_documents/ich_s6_r1.json`, `structured_data/derived/registry/manifest.json`, `working_docs/phase4_plan.md`, `working_docs/decisions.md` (DEC-051)

### DEC-054: Close DocumentEdition edition_role and document_status vocabularies; keep document_status distinct from effective currentness

- Date: 2026-07-13
- Status: Accepted
- Decision: `core.schema.json` adds two closed enums: `documentEditionRole` (`draft`, `final`, `revised_final`, `addendum`, `qa`, `annex`, `supplement`, `withdrawal_notice`, `replacement_notice`, `integrated_package`, `textually_consolidated_edition` — the Module 3.6 edition-role vocabulary) and `documentStatus` (`draft`, `in_force`, `withdrawn`, `superseded`). `document_edition.schema.json`'s `edition_role` and `document_status` properties reference these instead of the previously free-string `nonEmptyString`. `documentStatus` represents the edition's own publication lifecycle state, not a claim that the edition is the currently effective content for any jurisdiction or `as_of_date`; that determination remains EffectiveRecord/EffectiveStateSnapshot scope, computed from `document_status`, publication/effective dates, and lifecycle relationships (DEC-030). `LifecycleRelationship.relationship_type` remains an open free string; Module 4.2's production registry creates no LifecycleRelationship record (DEC-050), so closing that vocabulary is deferred to the module that first produces one (4.6 or 4.11).
- Rationale: Module 4.1 explicitly deferred these two DocumentEdition vocabularies to Module 4.2 ("Current free-string registry lifecycle fields are provisional until 4.2"). The production registry previously used `document_status="current"` for both M10 and S6 editions, which reads as an unqualified claim that the edition is the currently effective content — conflating the edition's own status with a computed effective-state fact the architecture reserves for later modules.
- Consequences: `structured_data/derived/registry/document_edition.json` and all `edition_role`/`document_status` values across `test/fixtures/derived_contract/` were updated to the closed vocabulary (`edition_role="parent"` becomes `"final"`; `document_status="current"` becomes `"in_force"`; `document_status="historical"` becomes `"superseded"` in the generic `complete_graph` two-edition example). No frozen Phase 1-3 artifact, source model `0.2.0`, or existing passing-fixture semantic intent changed; only the two free-string field values were normalized to the new closed vocabulary. Regression tests assert both enums are enforced and that the registry uses `in_force`, not a currentness value.
- Related files: `structured_data/schemas/derived/core.schema.json`, `structured_data/schemas/derived/artifacts/document_edition.schema.json`, `structured_data/derived/registry/document_edition.json`, `test/fixtures/derived_contract/`, `test/validate_derived.test.js`

### DEC-055: Add opt-in strict_registry validation mode, preserving generic partial-graph behavior

- Date: 2026-07-13
- Status: Accepted
- Decision: `scripts/validate_derived.js` accepts an optional manifest-level boolean field `strict_registry`. When `true`, `validateContractArtifacts` additionally requires every supplied GuidanceFamily to be referenced by at least one supplied DocumentEdition, and every supplied DocumentEdition to be referenced by at least one supplied EditionSource (`validateStrictRegistryCompleteness`). When absent or `false` (the default), validation behavior is unchanged from the existing generic partial-graph rules, which intentionally allow a fixture to supply only the artifacts relevant to what it demonstrates. `structured_data/derived/registry/manifest.json` sets `strict_registry: true`.
- Rationale: The generic contract-graph validator is deliberately permissive about partial artifact graphs so that test fixtures can exercise one relationship at a time without needing a complete registry. The production registry, however, is meant to be complete: every family should have at least one edition, and every edition should have a source-document link. Without an opt-in strict mode, an orphan GuidanceFamily or DocumentEdition could silently pass production registry validation.
- Consequences: All existing fixtures and tests that rely on partial artifact supply are unaffected (`strict_registry` defaults to off, and manifest validation requires `strict_registry` to be a boolean when present). The production registry manifest now fails validation if a future edit removes a family's only edition or an edition's only source link, without requiring RiskAssessment, ReviewAttestation, or any other governance-gated artifact type.
- Related files: `scripts/validate_derived.js`, `structured_data/derived/registry/manifest.json`, `test/validate_derived.test.js`

### DEC-056: Add EffectiveStateSnapshot member jurisdiction consistency validation

- Date: 2026-07-13
- Status: Accepted
- Decision: `validateContractSnapshots` additionally rejects a snapshot whose `effective_record_ids` member resolves to an EffectiveRecord with a different `jurisdiction` than the snapshot's own `jurisdiction`, alongside the existing `guidance_family_id` consistency check.
- Rationale: DEC-030 fixes snapshot identity as `guidance_family_id`, jurisdiction, `as_of_date`, review policy, derived contract version, source corpus identity, and calculation-policy version together. The Module 4.2 structural snapshot validation checked family consistency but not jurisdiction consistency, allowing a snapshot to silently mix EffectiveRecords from a jurisdiction other than its own declared jurisdiction.
- Consequences: This remains a structural, graph-level identity check, not the full snapshot disclosure/tier policy reserved for Module 4.8.
- Related files: `scripts/validate_derived.js`, `test/validate_derived.test.js`

### DEC-057: Harden manifest source_bundles configuration validation

- Date: 2026-07-13
- Status: Accepted
- Decision: `resolveManifestSourcePaths` in `scripts/validate_derived.js` now rejects, with a clean configuration error, a manifest `source_bundles`/`source_bundle_files` value that is present but not an array, an array containing any non-string or empty-string entry (reported per index), or an empty array. Previously such a manifest passed initial validation and later crashed with an uncaught `TypeError` from `path.resolve` when a non-string entry reached path resolution.
- Rationale: DEC-051 added the `source_bundles` manifest field without validating array element types, so a malformed manifest (for example, a typo producing a number or `null` entry) crashed the process instead of reporting a validation error, unlike every other manifest misconfiguration path.
- Consequences: `source_bundle`/`source_bundle_file` (singular) behavior is unchanged. `source_bundles`/`source_bundle_files` (plural) now fails cleanly with `configError: true` and a per-index message for malformed entries, an empty array, or a non-array value, instead of throwing.
- Related files: `scripts/validate_derived.js`, `test/validate_derived.test.js`

### DEC-058: Close EditionSource.source_role with a minimal justified vocabulary

- Date: 2026-07-13
- Status: Accepted
- Decision: `core.schema.json` adds a closed `sourceRole` enum with two values, `primary` and `supplementary`; `edition_source.schema.json`'s `source_role` property references it instead of the previously free-string `nonEmptyString`.
- Rationale: Module 3.6 defines EditionSource as a one-or-many link from a DocumentEdition to its source Document(s). A single-value vocabulary would make `source_role` a tautology (`primary` always, with no alternative), so the minimum vocabulary that gives the field meaning is two values: `primary` for an edition's principal source Document, and `supplementary` for an additional source Document jointly backing the same edition (for example a textually consolidated edition assembled from more than one source PDF). No repository data or plan text currently needs a third value, so none is added; `LifecycleRelationship.relationship_type` remains open for the same reason in the other direction (no evidence yet justifying closure — see the note added under the Module 4.2 non-goals).
- Consequences: All current `source_role` values (`"primary"`, used by every existing EditionSource record) already conform, so no fixture or production data changed. Regression tests assert the enum is enforced and that the production registry uses `primary`.
- Related files: `structured_data/schemas/derived/core.schema.json`, `structured_data/schemas/derived/artifacts/edition_source.schema.json`, `test/validate_derived.test.js`

### DEC-059: Defer closing LifecycleRelationship.relationship_type until a real inter-edition case exists

- Date: 2026-07-13
- Status: Accepted
- Decision: `LifecycleRelationship.relationship_type` remains an open, unconstrained non-empty string in `structured_data/schemas/derived/artifacts/lifecycle_relationship.schema.json`. It is deliberately not closed to an enum alongside `DocumentEdition.edition_role`/`document_status` (DEC-054) or `EditionSource.source_role` (DEC-058).
- Rationale: DEC-054 and DEC-058 close vocabularies that the Module 4.2 production registry actually populates with real data, giving direct evidence for the minimum justified set of values. No production LifecycleRelationship record exists: DEC-050 fixes the S6(R1) registry as one integrated-package edition with no inter-edition relationship, and the M10 registry has only one edition, so Module 4.2 creates zero LifecycleRelationship records. The only `relationship_type` values anywhere in the repository are placeholder strings (`"amends"`, `"same edition"`) in generic structural test fixtures (`test/fixtures/derived_contract/complete_graph/`, `.../valid/lifecycle_relationship.json`) that demonstrate the schema mechanics, not a real regulatory relationship. Closing the vocabulary now would mean inventing a taxonomy (for example `replaces`/`supersedes`/`withdraws`/`consolidates`) with no real case to validate it against, which is exactly the kind of ungrounded field-value population DEC-054 corrected for `document_status`.
- Consequences: `relationship_type` stays a free `nonEmptyString`. Closing it is deferred to the module that first produces a production LifecycleRelationship record — most likely Module 4.6 (derived artifact generation) or Module 4.11 (incremental family update), when a real edition-to-edition event (for example a revised S6 edition superseding this one) supplies genuine values to generalize a minimum vocabulary from. This decision records the deferral explicitly so it is not silently revisited without repository evidence.
- Related files: `structured_data/schemas/derived/artifacts/lifecycle_relationship.schema.json`, `working_docs/decisions.md` (DEC-050, DEC-054, DEC-058), `working_docs/phase4_plan.md`

### DEC-060: Extend strict_registry with required-artifact-type, disallowed-artifact-type, and duplicate-EditionSource-link checks

- Date: 2026-07-13
- Status: Accepted
- Decision: `validateStrictRegistryCompleteness` (DEC-055) gains three additional checks under `strict_registry: true`, all still opt-in and additive to the existing orphan-family and orphan-edition checks: (1) the supplied artifact graph must include at least one `GuidanceFamily`, at least one `DocumentEdition`, and at least one `EditionSource` artifact; (2) any supplied artifact of a type other than `GuidanceFamily`, `DocumentEdition`, or `EditionSource` (`LifecycleRelationship`, `AmendmentMapping`, `EffectiveRecord`, `EffectiveStateSnapshot`, `ReviewAttestation`, `RiskAssessment`) is rejected, one error per offending record; (3) two `EditionSource` records sharing the same `(document_edition_id, document_id, source_role)` triple are rejected as a duplicate link, while an edition may still carry distinct `primary` and `supplementary` links to different documents or the same document under different roles.
- Rationale: The original `strict_registry` mode (DEC-055) closed the "orphan record" gap (a supplied family/edition with nothing referencing it forward) but left three further gaps a genuinely strict registry validator should close: a manifest could omit an entire required artifact type (for example, no `EditionSource` artifact anywhere) without the existing orphan checks necessarily catching it, since those checks only fire when a record of the referencing type is present to be orphaned; a strict registry manifest could silently accumulate unrelated artifact types (risk, review, effective-state, lifecycle, amendment mapping) that Module 4.2 does not produce and that a genuinely strict registry manifest should not contain; and nothing previously prevented two identical `EditionSource` links from being declared for the same edition, document, and role, which is redundant registry data.
- Consequences: `structured_data/derived/registry/manifest.json` continues to pass unchanged (it supplies at least one of each required type, no disallowed types, and no duplicate EditionSource links). Default (non-strict) manifest and fixture behavior is completely unchanged, confirmed by regression tests showing the same scenarios (missing artifact types, disallowed artifact types, duplicate EditionSource links) still pass without `strict_registry: true`.
- Related files: `scripts/validate_derived.js`, `structured_data/derived/registry/manifest.json`, `test/validate_derived.test.js`
