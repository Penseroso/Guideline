...
Document status: Active. Decisions affecting this document are recorded in `working_docs/milestone_log.md` (one entry per milestone), not as individual `DEC-`/`REV-` entries — that convention is retired.
...
- What does *not* change: dedicated graph-database products (Neo4j etc.) stay deferred — the existing ID-referenced JSON graph (`CrossReference`/`Condition.applies_to_ids`) is already additive across accumulated files by design (each document is a self-contained bundle per `working_docs/schema.md`'s bundle contract, and cross-references resolve across the whole validated set) and doesn't degrade the way linear vector search does, so there's no equivalent migration trap to preempt there. A regulator-neutral derived-contract layer for amendment/effective-state tracking was explored early and archived, unused, at M1 (`working_docs/milestone_log.md`); if a second regulator's real data later needs this, design it fresh against the live schema rather than reviving the archived design.
...
- **M4 — Amendment/effective-state–aware answering.** An earlier "derived contract" layer exploring this (family/edition registry, AmendmentMapping, EffectiveRecord) was archived, unused, at M1 (`working_docs/milestone_log.md`) rather than kept paused indefinitely. If a second guideline's amendment/effective-state data actually needs this, design it fresh against the live schema and real M4 requirements rather than reviving the archived design.
