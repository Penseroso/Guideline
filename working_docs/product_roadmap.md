...
- **M1 — Chatbot MVP + extraction/verification pipeline, built on scale-ready primitives (current priority).** No new PDF extraction of *content* yet, but the pipeline that will do it is built and proven here: (1) index the existing `KnowledgeRecord`/`SourceUnit`/`Condition` objects in an embeddable vector store (selection criteria in §2.5) behind a thin retrieval interface, not a plain in-memory array; (2) build the schema-constrained extraction agent (§2.5) and the separate, narrower verification agent (§2.5.1), and dry-run both against a held-out slice of already-reviewed data (compare draft output to the existing human-reviewed version field-by-field). M1 is not done on this point until a per-field agreement rate is actually measured and reported — not "looks reasonable." The acceptance bar itself (e.g. what agreement rate is good enough to trust for M3) is set once that first real measurement exists, not guessed now with zero pipeline data; (3) stand up a small eval set of gold question/citation pairs and sampling-based drift monitoring (§2.5.1, §2.6) — this is the pipeline's only systemic-error detector now that no human reads every record, so it can't wait; (4) a query layer answering by structured lookup where possible (Option A, §2.2); (5) for questions that don't map to a structured lookup, retrieval over the vector store plus generation constrained to quote/cite retrieved objects (Option B, §2.2); (6) a citation-verification check before any answer is shown; (7) explicit "not found in the archive" refusal when nothing matches. Ship it as something a reviewer can actually type questions into (CLI is enough to start; no UI framework needed yet).
