...
- **M2 — Use it, watch where it fails.** Have real target users (internal reviewers) ask real questions. Log every refusal, and every answer that came from the grounded-RAG fallback rather than a direct structured-query hit (Option B vs. A, §2.2 — that distinction is the operational definition of "low-confidence" here, not a separately invented score). That log *is* the coverage-expansion backlog, replacing guesswork about which of ICH M10's ~70 sections (only 2 fully structured, 5 more probe-only, per the audit), S6(R1)'s ~41 sections (4 structured), or other-regulator documents to extract next. Done when: enough real questions have been logged to show recurring gap patterns rather than one-off gaps (a specific count/time-box is set once M1 is live and real query volume is known, not guessed here).
- **M3 — Extract content pulled by M2's actual gaps, using the M1 extraction + verification pipeline (no per-document human read-through).** This is where the two M10 pressure-probe files (`m10_phase2_table_pressure.json`, `m10_phase2_reference_condition_pressure.json`, sections `6.2`/`7.6`/`7.6.1`/`8`/`8.2`) get absorbed into full bundles — if and when real questions land there. If they don't, they stay as-is; retiring them is not an end in itself. New (non-ICH-M10/S6) source documents follow the same pipeline once the extraction and ingest steps are proven document-format-agnostic enough — format handling for FDA/EMA/OECD sources is not yet designed and is its own scoping decision when reached, not assumed here.
- **M4 — Amendment/effective-state–aware answering.** Resume extending the derived contract `0.1.0` layer (still paused, M0 in `working_docs/milestone_log.md`) only once a second guideline's amendment/effective-state data actually exists to justify it.
