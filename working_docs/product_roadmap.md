...
Document status: Active. Decisions affecting this document are recorded in `working_docs/milestone_log.md` (one entry per milestone), not as individual `DEC-`/`REV-` entries — that convention is retired and archived at `history/decisions.md`/`history/review_log.md`.

Supersedes: `history/phase3_plan.md`, `history/phase4_plan.md`, `history/phase4_handoff_plan.md`, `history/phase4_module_4_1.md` as the forward-looking plan. Those files remain the historical rationale trail for the derived contract those earlier phases explored (since archived, unused, at M1 — `history/README.md`); they no longer describe current intent. The non-goal boundary that excluded search/embeddings/RAG/web application (previously in `working_docs/project_scope.md`, `AGENTS.md`) is also superseded — see the updated versions of those files.

This document was drafted alongside an independent audit at `working_docs/audit_2026-08-18.md`. Read that first if you want the evidence; this document is the decision built on top of it.
...
- **Chunking destroys structure.** Splitting PDF text into fixed-size windows breaks the requirement/condition/exception/cross-reference linkage this project's schema (`working_docs/schema.md`) deliberately keeps structurally separate. A chunk boundary can separate a criterion from the exception that limits it.
...
- **Extraction: move to schema-constrained LLM extraction starting at M1, not deferred.** Manual curation (the source of the audited governance cost — roughly one governance entry per 2.5-2.7 structured records, `working_docs/audit_2026-08-18.md` §3) cannot cover a multi-regulator corpus — this isn't a trend preference, it's a volume requirement. Mechanics:
  ...
  4. Every LLM-produced record is schema-validated immediately with the existing validator (`scripts/validate_structured_data.js`) and starts at `review_status=needs_review` (the vocabulary already supports this — no schema change).
- What does *not* change: dedicated graph-database products (Neo4j etc.) stay deferred — the existing ID-referenced JSON graph (`CrossReference`/`Condition.applies_to_ids`) is already additive across accumulated files by design (each document is a self-contained bundle per `working_docs/schema.md`'s bundle contract, and cross-references resolve across the whole validated set — `history/decisions.md` DEC-018) and doesn't degrade the way linear vector search does, so there's no equivalent migration trap to preempt there. The regulator-neutral derived-contract layer (`structured_data/schemas/derived/`, paused per `working_docs/milestone_log.md` M0) stays paused until a second regulator's real data exists, but is no longer considered premature in principle — if the multi-regulator target holds, it will be needed, just not before real data justifies its shape.
...
The schema/extraction process is already proven (5 reviewed bundles, 126 passing tests, per the audit); what is *not* proven is the product mechanism itself (grounded retrieval, citation verification, refusal). **M1 builds the chatbot MVP on data that already exists.** Coverage expansion (M3) is not a prerequisite gate before that — it is pulled by real usage (M2) once the MVP shows which questions users actually ask and where the archive comes up empty. (Rationale for this ordering, including a same-day reversal of an earlier draft that got it backwards, is in `working_docs/milestone_log.md` M0 — not restated here.)

- **M0 — Foundation audit + roadmap.** Done. `working_docs/audit_2026-08-18.md` + this document.
- **M1 — Chatbot MVP + extraction/verification pipeline, built on scale-ready primitives (current priority).** No new PDF extraction of *content* yet, but the pipeline that will do it is built and proven here: (1) index the existing `KnowledgeRecord`/`SourceUnit`/`Condition` objects in an embeddable vector store (selection criteria in §2.5) behind a thin retrieval interface, not a plain in-memory array; (2) build the schema-constrained extraction agent (§2.5) and the separate, narrower verification agent (§2.5.1). **Primary validation is the verification agent's own entailment check against the real cited `source_text` — not agreement with old human-authored labels**, since the existing pilot data's correctness was itself unverified until it was audited (see below) and shouldn't be silently treated as ground truth. Comparing draft output to the existing human-reviewed version is a secondary, informative-only signal (does the new pipeline land near the established classification style), not a pass/fail gate. M1 is not done on this point until entailment-check accuracy is actually measured and reported against real source text — not "looks reasonable" and not "matched the old labels." The acceptance bar itself is set once that first real measurement exists, not guessed now with zero pipeline data; (3) stand up a small eval set of gold question/citation pairs and sampling-based drift monitoring (§2.5.1, §2.6) — this is the pipeline's only systemic-error detector now that no human reads every record, so it can't wait; (4) a query layer answering by structured lookup where possible (Option A, §2.2); (5) for questions that don't map to a structured lookup, retrieval over the vector store plus generation constrained to quote/cite retrieved objects (Option B, §2.2); (6) a citation-verification check before any answer is shown; (7) explicit "not found in the archive" refusal when nothing matches. Ship it as something a reviewer can actually type questions into (CLI is enough to start; no UI framework needed yet).
- **M2 — Use it, watch where it fails.** Have real target users (internal reviewers) ask real questions. Log every refusal, and every answer that came from the grounded-RAG fallback rather than a direct structured-query hit (Option B vs. A, §2.2 — that distinction is the operational definition of "low-confidence" here, not a separately invented score). That log *is* the coverage-expansion backlog, replacing guesswork about which of ICH M10's ~70 sections (2 fully structured; 2 more were touched by an earlier structural-probe pass, since archived at `history/` — see `working_docs/milestone_log.md` M1), S6(R1)'s ~41 sections (1 fully structured), or other-regulator documents to extract next. Done when: enough real questions have been logged to show recurring gap patterns rather than one-off gaps (a specific count/time-box is set once M1 is live and real query volume is known, not guessed here).
- **M3 — Extract content pulled by M2's actual gaps, using the M1 extraction + verification pipeline (no per-document human read-through).** M10 sections `6.2`/`7.6`/`7.6.1`/`8`/`8.2` were touched by an earlier schema-structural-probe pass (never full content coverage, and since archived — `working_docs/milestone_log.md` M1) — if real questions land there, extract them fresh against the live schema rather than reviving the archived probe files. New (non-ICH-M10/S6) source documents follow the same pipeline once the extraction and ingest steps are proven document-format-agnostic enough — format handling for FDA/EMA/OECD sources is not yet designed and is its own scoping decision when reached, not assumed here.
- **M4 — Amendment/effective-state–aware answering.** The earlier "derived contract `0.1.0`" layer exploring this (family/edition registry, AmendmentMapping, EffectiveRecord) was archived, unused, at M1 (`working_docs/milestone_log.md`) rather than kept paused indefinitely. If a second guideline's amendment/effective-state data actually needs this, design it fresh against the live schema and real M4 requirements rather than reviving the archived design — see `history/README.md`.
- **M5 — Productionize.** Deployment (reuse the existing Node.js stack — no new language/runtime unless M1-M4 demonstrate a real need), auth (scoped to the same internal-reviewer user base as §1.2, not a new public surface), monitoring (extend the M1 eval/drift-monitoring harness, §2.5.1, to live traffic rather than standing up a separate system), feedback loop (user-flagged wrong answers feed back into the eval set). Specific tooling choices are deferred to when M1-M4 are actually done, not picked now.

## 4. Governance

Logging convention and the repository cleanup verdict live in `AGENTS.md` (the active rule) and `working_docs/milestone_log.md` M0 (what was decided and why) — not restated here, since this document is about what to build, not how decisions get recorded.
