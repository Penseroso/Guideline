...
- Outputs: S6 PDF assessment, Phase 3 module plan, amendment and effective-state strategy, status document updates, and related decisions.
- Completion gate: Documentation agrees that Phase 3 has started, Module 3.0 is current scope, S6 cross-guideline validation is not complete, and model `0.2.0` remains unchanged.
...
- Outputs: Pilot-scope note identifying exact Part, section, page, and rationale for the selected S6 sample.
- Completion gate: Human-reviewable scope is approved and does not exceed the selected Parent and Addendum sections.
...
- Objective: Create a small source-preserving S6 pilot bundle using existing model `0.2.0`.
...
- Objective: Prototype reviewed mappings between Addendum and Parent `KnowledgeRecord` objects.
...
- Objective: Prototype reviewed current effective-state records derived from applicable Parent and Addendum records.
...
- Objective: Expand validation only after source-layer, amendment, or effective-state prototype evidence shows what checks are needed.
...
- Objective: Decide whether model `0.2.0` remains sufficient or requires a versioned extension for S6-derived layers.
