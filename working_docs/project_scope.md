...
Source model `0.2.0` remains unchanged. AmendmentMapping and `EffectiveRecord` Phase 3 prototypes are frozen historical derived-layer artifacts under `structured_data/derived/`, documented in `working_docs/amendment_effective_strategy.md`, `working_docs/amendment_prototype_S6_R1.md`, and `working_docs/effective_state_prototype_S6_R1.md`. They remain outside the source JSON Schema and are validated separately by `scripts/validate_legacy_derived.js`. Module 3.6 documents the provisional derived-layer contract boundary in `working_docs/derived_contract_module_3_6.md`. Phase 4 Module 4.1 is complete after REV-012, with production derived contract validation isolated in `scripts/validate_derived.js`. Phase 4 Rebaseline R0, a pre-Module-4.2 architecture audit recording decisions DEC-049 through DEC-052, is complete after REV-013a; it changed no schema, structured data, or validator behavior. Module 4.2 is complete after REV-013b: production registry artifacts exist for both corpus documents under `structured_data/derived/registry/`, the derived validator additively supports multi-bundle manifests and structural EffectiveStateSnapshot validation, and two candidate-only test fixtures demonstrate the registry-to-derived-to-snapshot value path against real M10 and S6 pilot content, all at `review_status=needs_review`. Module 4.2 was independently audited and corrected after REV-013c (DEC-053 through DEC-057): the registry's source-Document identity now comes from canonical minimal bundles under `structured_data/source_documents/` rather than `structured_data/pilots/`, `DocumentEdition.edition_role`/`document_status` use closed vocabularies distinct from computed effective currentness, registry validation is strict (`strict_registry: true`), snapshot validation checks jurisdiction as well as family consistency, and manifest `source_bundles` configuration errors are reported cleanly. A further hardening pass after REV-013d (DEC-058 through DEC-060) closed `EditionSource.source_role` to a minimal `primary`/`supplementary` vocabulary, extended `strict_registry` to require at least one artifact of each registry type, reject artifact types outside the registry, and reject duplicate EditionSource links, and explicitly recorded `LifecycleRelationship.relationship_type` as deferred pending a real inter-edition case rather than closed without evidence. Module 4.3 is not started and now eligible to begin.
