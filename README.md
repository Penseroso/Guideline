...
- The Module 3.3 amendment-mapping prototype is a provisional derived-layer artifact under `structured_data/derived/`, outside the JSON Schema and validator; the `EffectiveRecord` effective-state layer remains an unimplemented derived-layer design.
- S6 effective-state records, schema changes, full extraction, database, search, embeddings, RAG, web application, and regulatory decision automation are not started.
...
- `structured_data/`: machine-readable structured outputs and schemas; `structured_data/derived/` holds provisional derived-layer artifacts (amendment mappings) that are outside the source JSON Schema and validator.
...
- `working_docs/amendment_prototype_S6_R1.md`: implemented Module 3.3 amendment-relation prototype note, pending repository review REV-005; sample at `structured_data/derived/s6_r1_amendment_mappings.json`.
